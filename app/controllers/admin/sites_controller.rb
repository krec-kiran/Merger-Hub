class Admin::SitesController < ApplicationController
  ...
  def index
    @sites = Site.includes(:geo, :sector).order('updated_at DESC').page(params[:page]).per(10)
  end
  ...
  private
    ...
    def set_site
      @site = Site.includes(:geo, :sector).find(params[:id])
    end

    def site_params
      params.require(:site).permit(:name, :url, :is_sellside, :is_buyside, :is_movers, :sector_id, :geo_id, :size_of_deal)
    end
end